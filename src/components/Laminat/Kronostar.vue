<template>
  <div>
    <!-- BREADCRUMBS -->    
      ...
      <!-- Bottom Navigations -->
       <bottom-navigation 
        :list_collection="getColKronostar"
...
export default {
  name: "Kronostar",
  data: () => ({
     brand:'ЛАМИНАТ Kronostar',
     jut: [],
    bottomNav: "Grünhof 4V",
    bread_items: [
      {
        text: "Главная",
        disabled: false,
  ...
  async mounted() {
    if (!this.jut.length) this.jut = await this.$store.dispatch("fetchKronostar");
  },
  computed: {
    ...mapGetters(["getKronostar", "getColKronostar"]),
    filteredProd() {
      let products = this.getKronostar;
      if (this.bottomNav)
